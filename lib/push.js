...
function buildScript(address, data, parentTxID) {
...
async function createNode(xprivKey, data, parentTxID, keyPath) {
  xprivKey = bsv.HDPrivateKey.fromString(xprivKey)
  keyPath = keyPath || 'm/0'
  const address = xprivKey.deriveChild(keyPath).publicKey.toAddress().toString()
  const script = buildScript(address, data, parentTxID)
  const fee = estimateFee(script)
  const fundingKey = xprivKey.privateKey.toString()
  let privKey = fundingKey

  if (parentTxID) {
    const parentPrivKey = xprivKey.deriveChild(getParentPath(keyPath)) 
    ...
    const fundingBalance = await getBalance(xprivKey.publicKey.toAddress().toString())
  
    if (fundingBalance < fee) { throw `Not enough money (${fundingBalance} < ${fee})` }
  }

  const response = await pushNode(privKey, script)
  if (!response.txid) {
