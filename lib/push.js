...
async function createNode({xPrivKey, data = [], parentTxID, keyPath = 'm/0', safe = true}) {

  if (!xPrivKey) throw 'No extended private key provided'
  const hdPrivateKey = bsv.HDPrivateKey.fromString(xPrivKey)
  const address = hdPrivateKey.deriveChild(keyPath).publicKey.toAddress().toString()
  const script = buildScript(address, data, parentTxID, safe)
  const fee = estimateFee(script)
  const fundingKey = hdPrivateKey.privateKey.toString()
