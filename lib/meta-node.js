...
class MetaNode {
  ...
  async createChild(xprivKey, data, keyPath) {
    if (!keyPath) {
      keyPath = await this.getUnusedChildKeyPath()
    }
    return await push.createNode(xprivKey, data, this.txid, keyPath)
  }

  async createUpdate(xprivKey, data, keyPath) {
    keyPath = keyPath || this.getDefaultKeyPath()
    return await push.createNode(xprivKey, data, this.tx.parent.txid, keyPath)
  }
}
